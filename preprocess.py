...
KERN_DATASET_PATH = "/home/ubuntu/generating-melodies-with-rnn-lstm/Preprocessing-dataset-for-melody-generation-pt-1/code/deutschl/test/"
SAVE_DIR = "dataset"
...
def transpose(song):
  """Analyzes song key and keeps it within major or minor tonality.
  ...
  if not (key.mode == "major" or key.mode == "minor"):
    ...
    return None  # Or you can choose to handle non-major/minor songs differently

  # Keep the song in its original key (no transposition)
  return song
...
def generate_training_sequences(sequence_length):
    ...
    one_hot_inputs = F.one_hot(inputs_tensor, num_classes=vocabulary_size)
    one_hot_inputs = one_hot_inputs.numpy() 
    
    targets = np.array(targets)

    print(f"There are {len(inputs)} sequences.")

    return one_hot_inputs, targets
'''
