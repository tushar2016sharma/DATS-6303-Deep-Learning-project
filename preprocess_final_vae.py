...
KERN_DATASET_PATH = "/home/ubuntu/generating-melodies-with-rnn-lstm/3 - Preprocessing dataset for melody generation pt 1/code/deutschl/erk"
SAVE_DIR = "dataset"
...
def main():
    ...
    if os.listdir(SAVE_DIR):  # Check if the directory is not empty
        file_path = create_single_file_dataset(SAVE_DIR, SINGLE_FILE_DATASET, SEQUENCE_LENGTH)
        create_mapping(file_path, MAPPING_PATH)
        # Example of passing device to function
        for batch_inputs, batch_targets, vocab_size in generate_training_sequences(SEQUENCE_LENGTH, BATCH_SIZE, device):
            # Your training loop or further processing here
            pass
    else:
        print("No songs were processed and saved. Please check the input dataset.")
