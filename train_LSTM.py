...
def train_model():
    ...
    # If targets are one-hot encoded or not in the correct format, convert them to class indices
    if targets.ndim > 1 and targets.size(1) > 1:
        targets = targets.argmax(dim=1)  # Assuming that targets are one-hot encoded
    ...
    # Model
    model = MusicModel(INPUT_UNITS, NUM_UNITS, OUTPUT_UNITS).to(device)
